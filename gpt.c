...
/* Example code to drive a 4 digit 7-segment numeric LED backpack using a HT16K33 I2C
    driver chip
...
// Converts a character to the bit pattern needed for a 7-segment display.
// Bit mapping (LSB..): A,B,C,D,E,F,G,DP
uint16_t char_to_pattern(char ch) {
    ...
    // Support common extra characters
    if (ch == ' ')
        return 0;
    if (ch == '-')
        return 0x40; // G segment
    if (ch == '.')
        return 0x80; // DP
    ...
    // Support basic hex A-F (uppercase)
    switch (toupper((unsigned char)ch)) {
        case 'A': return 0x77;
        case 'B': return 0x7C; // lowercase b style
        case 'C': return 0x39;
        case 'D': return 0x5E;
        case 'E': return 0x79;
        case 'F': return 0x71;
...
static inline void ht16k33_display_set(int position, uint16_t bin) {
    ...
    // Some backpacks wire extra segments for the last digit into the high
    // byte only. As a compatibility attempt, duplicate the low byte into
    // the high byte for position 3 so both halves receive the same pattern.
    if (position == 3) {
        buf[2] = buf[1];
    }
#ifdef i2c_default
...
#ifdef DEBUG_DISPLAY
    printf("HT16K33 write pos=%d addr=0x%02x b0=0x%02x b1=0x%02x\n", position, buf[0], buf[1], buf[2]);
#endif
#endif
}
...
again:
    ...
    ht16k33_scroll_string("0123456789", 150);
